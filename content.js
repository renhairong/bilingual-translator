...
// 归一化 host：用 Public Suffix List（psl）精确提取主域名
// 支持所有公共后缀：github.io / co.uk / com.cn / com.hk 等
// 优于手写穷举列表——psl 是 Mozilla 维护的完整公共后缀表
...
function normalizeHost(host) {
  ...
  const hostOnly = h.split(':')[0]; // 去掉端口
  if (hostOnly === 'localhost' || /^\d+\.\d+\.\d+\.\d+$/.test(hostOnly)) return h;
  try {
    // psl.parse 返回 { domain, sld, tld }，domain 即注册主域
    // 纯后缀（如 github.io、co.uk）domain 为 null → 保持原样不动
    const r = psl.parse(hostOnly);
    if (r && r.domain) return r.domain.toLowerCase();
  } catch (e) { /* 解析失败退回原始 host */ }
  return h;
}
